...
object Huffman {
  ...
  def until(singletonFunc: List[CodeTree] => Boolean, combineFunc: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] =
    if (singletonFunc(trees)) trees else until(singletonFunc, combineFunc)(combineFunc(trees))
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def decodeHelper(subTree: CodeTree, bits: List[Bit], textAcc: List[Char]): List[Char] = subTree match {
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def encodeChar(tree: CodeTree, char: Char, bitsAcc: List[Bit]): List[Bit] = tree match {
  ...
  def convert(tree: CodeTree): CodeTable = {
    def convertHelper(tree: CodeTree, bitsAcc: List[Bit], tableAcc: CodeTable): CodeTable = tree match {
      case Fork(left, right, _, _) =>
        mergeCodeTables(convertHelper(left, 0 :: bitsAcc, tableAcc), convertHelper(right, 1 :: bitsAcc, tableAcc))
      case Leaf(char, _) => (char, bitsAcc.reverse) :: tableAcc
    }
